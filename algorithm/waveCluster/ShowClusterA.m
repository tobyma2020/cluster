...
for i=1:N
    ...
    if(~isempty(ir))
        if col>3
            scatter3(A(ir,1),A(ir,2),A(ir,3),'MarkerFaceColor',lineStyles(i+1,:));
        else
            scatter(A(ir,1),A(ir,2),60,'MarkerFaceColor',lineStyles(i+1,:),'MarkerEdgeColor',lineStyles(i+1,:),'Marker','.');          
